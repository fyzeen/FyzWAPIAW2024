#!/bin/bash -l
#SBATCH --job-name=HCP_YA_asegYeo_oneConfound
#SBATCH --account=janine_bijsterbosch
#SBATCH --output="/scratch/l.lexi/WAPIAW2024/Experiments/HCP_team/hcp_ya/logs/HCP_YA_asegYeo_oneConfound.out%j"
#SBATCH --error="/scratch/l.lexi/WAPIAW2024/Experiments/HCP_team/hcp_ya/logs/HCP_YA_asegYeo_oneConfound.err%j"
...
# input parameters
IDP_filepath="${dataset_dir}/aseg_yeo.csv"
...
outname="HCP_YA_asegYeo_intraCranialVolConfound"
...
### NOTE: TO CHOOSE WHICH CONFOUND, EDIT THIS FILE!!! ###
...
python ${reg_script} -x "${IDP_filepath}" -y "${phenotype_filepath}" -c "${confound_filepath}" -s "${sublist_filepath}" -C "${cnflist_filepath}" -o "${outdir}" -N "${outname}" -v
