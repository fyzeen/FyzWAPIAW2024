...
def _get_metadata(X_fpath, Y_fpath, cnfvars_fpath=None, idplist_fpath=None, ptylist_fpath=None, cnflist_fpath=None):
    dataset_name = os.path.dirname(X_fpath.split("Data/")[1]).split(os.sep)[0]
    if 'func_idp' in X_fpath:
        datatype = os.path.basename(X_fpath).split('.')[0]
    else:
        datatype = os.path.basename(X_fpath).split('.')[0].split("_")[0]
